...
def build_graph_2(tf_image_batch, tf_labels, tf_keep_prob, lr=1.0):
    ...
    with graph.as_default():
        ...
        conv3 = conv_layer(pool1, [3, 3, 16, 16], stride=2, padding="VALID")
        conv4 = conv_layer(
            conv3, [3, 3, 16, 16], padding="VALID", stride=2, act_fun=tf.nn.relu
        )
        ...
        conv5 = conv_layer(pool2, [1, 1, 16, 32], padding="VALID", act_fun=tf.nn.relu)
        conv6 = conv_layer(conv5, [1, 1, 32, 64], act_fun=tf.nn.relu)
        flat_conv6 = tf.reshape(
            conv6, shape=[-1, reduce(lambda x, y: x * y, conv6.shape.as_list()[1:], 1)]
        )
        fc1 = fc_layer(flat_conv6, 64, act_fun=tf.nn.relu)
        drop_2 = tf.nn.dropout(fc1, keep_prob=tf_keep_prob)
        fc2 = fc_layer(drop_2, 32, act_fun=tf.nn.relu)
